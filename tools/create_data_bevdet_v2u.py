...
def add_ann_adj_info(extra_tag):
    ...
    dataroot = 'data/V2U-COO-TEST/'
    nuscenes = NuScenes(nuscenes_version, dataroot)
    for set in ['val']:
        dataset = pickle.load(
            open('data/V2U-COO-TEST/%s_infos_%s.pkl' % (extra_tag, set), 'rb'))
        for id in range(len(dataset['infos'])):
            ...
            dataset['infos'][id]['occ_path'] = \
                'data/V2U-COO-TEST/gts/%s/%s'%(scene['name'], info['token'])
        with open('data/V2U-COO-TEST/%s_infos_%s.pkl' % (extra_tag, set),
                  'wb') as fid:
