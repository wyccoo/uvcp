...
def parse_args():
    ...
    parser.add_argument(
        '--root_path',
        type=str,
        default='../autodl-tmp/cooperative-vehicle-infrastructure/',
...
def main():
    ...
    if args.format == 'video':
        ...
        vout = cv2.VideoWriter(
            ...
            args.fps, (int(1920 / scale_factor * 3),
                       int(1080 / scale_factor * 2 + canva_size)))
    ...
    for cnt, infos in enumerate(
        ...
        for rid in range(len(pred_res)):
            pred_res[rid]['translation'][1] += 3152
        pred_boxes = [
        ...
        if args.draw_gt:
            ...
            corners_lidar_gt = \
                LB(infos['gt_boxes'],
                   origin=(0.5, 0.5, 0)).corners.numpy().reshape(-1, 3)
        ...
        img = np.zeros((1080 * 2 + canva_size * scale_factor, 1920 * 2, 3),
                       dtype=np.uint8)
        # import pdb;pdb.set_trace()
        img[:1080, :, :] = np.concatenate(imgs[:2], axis=1)
        img_back = np.concatenate(
            [imgs[0][:, ::-1, :], imgs[1][:, ::-1, :]],
            axis=1)
        img[1080 + canva_size * scale_factor:, :, :] = img_back
        img = cv2.resize(img, (int(1920 / scale_factor * 3),
                               int(1080 / scale_factor * 2 + canva_size)))
        w_begin = int((1920 * 3 / scale_factor - canva_size) // 2)
        img[int(1080 / scale_factor):int(1080 / scale_factor) + canva_size,
            w_begin:w_begin + canva_size, :] = canvas
        if args.format == 'image':
            cv2.imwrite(os.path.join(vis_dir, '%s.jpg' % i), img)
            i+=1
