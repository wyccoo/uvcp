...
class LoadAnnotationsBEVDepth(object):
    ...
    def __call__(self, results):
        gt_boxes = results['curr']['gt_boxes']
        gt_velocity = results['curr']['gt_velocity']
        ...
        results['gt_bboxes_3d'] = \
            LiDARInstance3DBoxes(gt_boxes, box_dim=gt_boxes.shape[-1],
                                 origin=(0.5, 0.5, 0.5))
        length = gt_boxes.size(0)

        # 生成一个与 gt_boxes 长度相同的 tensor
        # 这里创建一个初始化为零的 tensor
        results['gt_labels_3d'] = torch.zeros(length)
        imgs, rots, trans, intrins = results['img_inputs'][:4]
