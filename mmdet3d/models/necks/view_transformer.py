...
class AttentionModule(nn.Module):
    def __init__(self, in_channels, out_channels, l2_reg=1e-5):
        ...
        self.adjustment_factor = nn.Parameter(torch.tensor(0.4))
    ...
    def forward(self, x1, x2):
        # 将两个张量拼接在通道维度上
        # x1_num = torch.sum(x1, dim=1, keepdim=True)
        # x1_num = x1_num.cpu().detach().numpy()
        # x2_num = torch.sum(x2, dim=1, keepdim=True)
        # # x2_num = x2_num.cpu().detach().numpy()
        # fpn_layer = FFPN(in_channels=80, out_channels=80)
        # device = x2.device
        # fpn_layer = fpn_layer.to(device)
        # ups1, ups2, ups3, ups4 = fpn_layer(x2)
        # bev_feat_veh = x1.sum(dim=(-1, -2))
        ...
        spatial_weights = self.spatial_attention(concatenated_tensor)

        # 使用调整因子进行插值
        spatial_weights_x1 = self.adjustment_factor * spatial_weights[:, :x1.size(1), :, :]
        spatial_weights_x2 = (1 - self.adjustment_factor) * spatial_weights[:, x1.size(1):, :, :]

        # 将权重应用到输入上
        attended_features_x1 = x1 * spatial_weights_x1
        attended_features_x2 = x2 * spatial_weights_x2

        # 添加 L2 正则化项
        l2_regularization = torch.tensor(0.0, device=x1.device)
        for param in self.parameters():
            l2_regularization += torch.norm(param, p=2)

        # 将两部分特征相加作为最终输出
        final_output = attended_features_x1 + attended_features_x2 + 1e-5 * l2_regularization
        # final_output = attended_features_x1 + attended_features_x2
        # f_num = torch.sum(final_output, dim=1, keepdim=True)
        # f_num = f_num.cpu().detach().numpy()
...
class LSSViewTransformer(BaseModule):
    ...
    def get_lidar_coor(self, sensor2ego, ego2global, cam2imgs, post_rots, post_trans,
        ...
        a = bda.view(B, 1, 1, 1, 1, 3, 3)
        b = points.unsqueeze(-1)
...
@NECKS.register_module()
class LSSViewTransformerveh(BaseModule):
    r"""Lift-Splat-Shoot view transformer with BEVPoolv2 implementation.
    ...
    def __init__(
        ...
        super(LSSViewTransformerveh, self).__init__()
        self.grid_config = grid_config
        self.downsample = downsample
        self.create_grid_infos(**grid_config)
        self.sid = sid
    ...
    def forward(self, input):
        ...
        x = x.view(B * N, C, H, W)
        x = self.depth_net(x)
        depth_np = x.detach().cpu().numpy()
        # batch_size, num_classes, height, width = x.shape
        # num_classes = num_classes - self.out_channels
        depth_digit = x[:, :self.D, ...]
        tran_feat = x[:, self.D:self.D + self.out_channels, ...]
        depth = depth_digit.softmax(dim=1)
        device = depth.device
        ...
        # 获取图像尺寸
        batch_size, num_classes, height, width = depth_numpy.shape

        # 将每个像素点的深度估计映射回深度值
        depth_values = np.argmax(depth_numpy, axis=1)

        d = densecrf.DenseCRF2D(width, height, num_classes)
        probabilities = []
        for i in range(batch_size):
            U = unary_from_labels(depth_values[i], num_classes, 0.1)
            d.setUnaryEnergy(U)
            d.addPairwiseGaussian(sxy=(3, 3), compat=3)
            Q = d.inference(10)  # 使用信念传播进行5次推断

            # 获取优化后的概率图
            probabilities.append(np.array(Q).reshape((num_classes, height, width)))

            # 获取深度估计结果
            crf_output_depth_map = np.argmax(probabilities, axis=0) + 1
        probabilitie = np.stack([probabilities[i] for i in range(batch_size)], axis=0)
        probabilitie = 0.1 * probabilitie + 0.9 * depth_numpy
        depth = torch.tensor(probabilitie).to(device)
...
@NECKS.register_module()
class LSSViewTransformeruav(BaseModule):
    r"""Lift-Splat-Shoot view transformer with BEVPoolv2 implementation.
    ...
    def __init__(
        ...
        super(LSSViewTransformeruav, self).__init__()
        self.grid_config = grid_config
    ...
    def get_lidar_coor(self, sensor2ego, ego2global, cam2imgs, post_rots, post_trans,
        ...
        # post-transformation
        # B x N x D x H x W x 3
        points = self.frustum.to(sensor2ego)
        points = self.frustum.to(sensor2ego) - post_trans.view(B, N, 1, 1, 1, 3)
        points = torch.inverse(post_rots).view(B, N, 1, 1, 1, 3, 3)\
            .matmul(points.unsqueeze(-1))
    ...
    def forward(self, input):
        ...
        depth = depth_digit.softmax(dim=1)
        # device = depth.device
        # depth1 = self.get_lidar_coor(*input[1:7])
        # depth2 = torch.round(torch.sqrt(depth1[..., 0]**2 + depth1[..., 1]**2 + depth1[..., 2]**2)).view(B * N, self.D, H, W)
        # depth_numpy = depth.detach().cpu().numpy()
        # # 获取图像尺寸
        # batch_size, num_classes, height, width = depth2.shape
        #
        # # 将每个像素点的深度估计映射回深度值
        # depth_values1 = np.argmax(depth_numpy, axis=1).reshape(batch_size, 1, height, width)
        # depth_values = depth2.detach().cpu().numpy()
        # # depth_values = depth_values[:, depth_values1[:,]]
        # depth_values = depth_values.astype('int64')
        # depth_values = np.take_along_axis(depth_values, depth_values1, axis=1)
        #
        # d = densecrf.DenseCRF2D(width, height, 200)
        # probabilities = []
        # for i in range(batch_size):
        #     U = unary_from_labels(depth_values[i], 200, 0.7)
        #     d.setUnaryEnergy(U)
        #     d.addPairwiseGaussian(sxy=(3, 3), compat=3)
        #     Q = d.inference(10)  # 使用信念传播进行5次推断
        #
        # # 获取优化后的概率图
        #     probabilities.append(np.array(Q).reshape((200, height, width)))
        #
        #     # 获取深度估计结果
        #     crf_output_depth_map = np.argmax(probabilities, axis=0)
        # probabilitie = np.stack([probabilities[i] for i in range(batch_size)], axis=0)
        # probabilitie = probabilitie[:, 20:70, :, :]
        # probabilitie = 0.1 * probabilitie + 0.9 * depth_numpy
        # depth = torch.tensor(probabilitie).to(device)
        device = depth.device
        depth_numpy = depth.detach().cpu().numpy()
