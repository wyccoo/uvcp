...
class BEVDet(CenterPoint):
    ...
    def __init__(self, img_view_transformer_veh, img_view_transformer_uav, img_attention, img_bev_select, img_bev_encoder_backbone,
                 img_bev_encoder_neck, **kwargs):
        super(BEVDet, self).__init__(**kwargs)
        self.img_view_transformer_veh = builder.build_neck(img_view_transformer_veh)
        self.img_attention = builder.build_neck(img_attention)
        self.img_view_transformer_uav = builder.build_neck(img_view_transformer_uav)
        self.img_bev_encoder_backbone = \
            builder.build_backbone(img_bev_encoder_backbone)
        self.img_bev_encoder_neck = builder.build_neck(img_bev_encoder_neck)
        self.img_bev_selector = builder.build_neck(img_bev_select)
...
class BEVDet4D(BEVDet):
    ...
    def gen_grid(self, input, sensor2keyegos, bda, bda_adj=None):
        ...
        feat2bev[0, 0] = self.img_view_transformer_uav.grid_interval[0]
        feat2bev[1, 1] = self.img_view_transformer_uav.grid_interval[1]
        feat2bev[0, 2] = self.img_view_transformer_uav.grid_lower_bound[0]
        feat2bev[1, 2] = self.img_view_transformer_uav.grid_lower_bound[1]
        feat2bev[2, 2] = 1
    ...
    def prepare_bev_feat(self, img, rot, tran, intrin, post_rot, post_tran,
        ...
        _, N, _, _, _ = x.shape
        x_split = torch.split(x, split_size_or_sections=1, dim=1)
        rot_split = torch.split(rot, split_size_or_sections=1, dim=1)
        tran_split = torch.split(tran, split_size_or_sections=1, dim=1)
        intrin_split = torch.split(intrin, split_size_or_sections=1, dim=1)
        post_rot_split = torch.split(post_rot, split_size_or_sections=1, dim=1)
        post_tran_split = torch.split(post_tran, split_size_or_sections=1, dim=1)
        bev_feat_split = []
        depth_split = []
        for i in range(N):
            if i == 0:
                bev_veh, depth_split_n = self.img_view_transformer_veh(
                    [x_split[i], rot_split[i], tran_split[i], intrin_split[i], post_rot_split[i], post_tran_split[i],
                     bda, mlp_input])

            else:
                bev_feat_n, depth_split_n = self.img_view_transformer_uav(
                    [x_split[i], rot_split[i], tran_split[i], intrin_split[i], post_rot_split[i], post_tran_split[i],
                     bda, mlp_input])
                bev_feat_split.append(bev_feat_n)
                depth_split.append(depth_split_n)
        # bev_feat = self.allain_bev_feat(bev_feat_split)
        device = img.device
        # zero_mask_0 = bev_veh == 0
        # non_zero_0 = ~(zero_mask_0)
        # veh_tensor1 = torch.zeros_like(bev_feat_split[0])
        # veh_tensor2 = torch.zeros_like(bev_feat_split[0])
        # veh_tensor1[non_zero_0] = bev_feat_split[0][non_zero_0]  # 将掩码位置上的元素置为 tensor2 对应位置的值，其余置为零
        # veh_tensor2[non_zero_0] = bev_feat_split[1][non_zero_0]  # 将掩码位置上的元素置为 tensor3 对应位置的值，其余置为一
        # similarity_with_tensor2 = F.cosine_similarity(bev_veh.view(bev_veh.size(0), -1),
        #                                               veh_tensor1.view(veh_tensor1.size(0), -1),
        #                                               dim=1)
        # similarity_with_tensor3 = F.cosine_similarity(bev_veh.view(bev_veh.size(0), -1),
        #                                               veh_tensor2.view(veh_tensor2.size(0), -1),
        #                                               dim=1)
        # ratios1 = similarity_with_tensor2 / (similarity_with_tensor2 + similarity_with_tensor3)
        # ratios2 = similarity_with_tensor3 / (similarity_with_tensor2 + similarity_with_tensor3)
        # # 对两个张量进行加权求和
        # bev_sum = ratios1.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) * bev_feat_split[0] + ratios2.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) * bev_feat_split[1]
        # bev_feat_veh = torch.zeros_like(bev_feat_split[0])
        # bev_feat_veh[non_zero_0] = bev_veh[non_zero_0]
        # bev_feat_veh[zero_mask_0] = bev_sum[zero_mask_0]
        # # 在最后两个维度上求和，得到最终的结果
        # # bev_feat_veh = weighted_sum.sum(dim=(-1, -2))

        # zero_mask_1 = bev_feat_split[0] == 0
        # zero_mask_2 = bev_feat_split[1] == 0
        # # 计算非零值的位置，即两个张量都不为零的位置
        # non_zero_mask = ~(zero_mask_1 | zero_mask_2)

        # # 在非零值的位置进行平均
        # result_tensor = torch.zeros_like(bev_feat_split[0])
        # result_tensor[non_zero_mask] = (bev_feat_split[0][non_zero_mask] + bev_feat_split[1][non_zero_mask]) / 2

        # # 在一个张量为零的位置只进行相加，不做平均
        # result_tensor[zero_mask_1] = bev_feat_split[1][zero_mask_1]
        # result_tensor[zero_mask_2] = bev_feat_split[0][zero_mask_2]
        # result_tensor.to(device)
        # bev_feat_uav = result_tensor
        bev_feats_uav = []
        bev_feats_veh = []
        bev_feats_uav.append(bev_feat_split[0])
        bev_feats_veh.append(bev_veh)

        bev_feat_vehs, bev_feat_uav = self.img_bev_selector(bev_feats_veh, bev_feats_uav)
        # 应用注意力机制进行特征融合

        bev_feat = self.img_attention(bev_feat_vehs, bev_feat_uav).to(device)
        # bev_feat = self.img_attention(bev_veh, bev_feat_uav).to(device)
        # bev_feats = []
        # bev_feats.append(bev_feat_vehs)
        # bev_feats.append(bev_feat_uav)
        # bev_feat = torch.mean(torch.stack(bev_feats), dim=0)
        depth = depth_split[0]
        if self.pre_process:
            bev_feat = self.pre_process_net(bev_feat)[0]
        return bev_feat, depth

    def allain_bev_feat(self, bev_feat):
        N, _, _, _ = bev_feat[0].shape
        bev_feat_base = bev_feat[0]
        bev_feat_base_list = [bev_feat_base[j, :, :, :] for j in range(N)]
        bev_feat_base = bev_feat_base_list[0]
        bev_feat_allain = bev_feat[1]
        bev_feat_allain_list = [bev_feat_allain[k, :, :, :] for k in range(N)]
        bev_feat_allain = bev_feat_allain_list[0]
        # original_shape = bev_feat_allain.shape
        # print(original_shape)
        # z_values = bev_feat_allain.reshape(original_shape[0], -1)
        # interp_func = interp2d(np.arange(original_shape[1]), np.arange(original_shape[2]), bev_feat_allain[0], kind='linear',
        #                        fill_value=0)
        max_similarity = -1
        most_similar_index = None
        # select = [1, 144/128, 184/128, 192/128]
        select = [1, 192/128]
        for scaling_factor in select:
            # 计算插值比例
            zoom_factor = (1, scaling_factor, scaling_factor)

            # 进行双线性插值
            # interpolated_tensor = zoom(bev_feat_allain.cpu().detach().numpy(), zoom_factor, order=1)
            # interpolated_tensor = torch.tensor(interpolated_tensor, device=bev_feat_allain.device,
            #                                    dtype=bev_feat_allain.dtype)
            interpolated_tensor = F.interpolate(bev_feat_allain.unsqueeze(0).unsqueeze(0), scale_factor=zoom_factor,
                                                mode='nearest').squeeze(0).squeeze(0)

            # target_shape = (80, 128, 128)
            shape1 = bev_feat_base.shape
            shape2 = interpolated_tensor.shape
            # assert target_shape == shape1

            # 初始化最相似性和最相似的索引
            max_similarity_branch = -1
            most_similar_index_branch = None

            # 计算相似性并找到最相似的部分
            for i in range(0, shape2[1] - shape1[1] + 1, 8):
                for j in range(0, shape2[2] - shape1[2] + 1, 8):
                    window2 = interpolated_tensor[:, i:i + shape1[1], j:j + shape1[2]]
                    # similarity = F.cosine_similarity(bev_feat_base.view(1, -1), window2.view(1, -1))[0, 0]
                    # 确保余弦相似性计算时张量的维度匹配
                    bev_feat_base_flat = bev_feat_base.cpu().detach().numpy().flatten().reshape(1, -1)
                    window2_flat = window2.cpu().detach().numpy().flatten().reshape(1, -1)

                    # 计算余弦相似性
                    similarity = cosine_similarity(bev_feat_base_flat, window2_flat)[0, 0]
                    if similarity > max_similarity_branch:
                        max_similarity_branch = similarity
                        most_similar_index_branch = (i, j)
                        a = scaling_factor
            if max_similarity_branch > max_similarity:
                interpolated_tensor_usefull = interpolated_tensor
                max_similarity = max_similarity_branch
                most_similar_index = most_similar_index_branch
                bev_uav_list = []
                num = a
                print(max_similarity)
                print(most_similar_index_branch)
                print(num)

        # 确定填充的数量
        pad_width = ((0, 0),  # 在第一个维度上填充0行
                     (most_similar_index[0], interpolated_tensor_usefull.shape[1] - most_similar_index[0] - bev_feat_base.shape[1]),
                     # 在第二个维度上填充对应位置
                     (most_similar_index[1], interpolated_tensor_usefull.shape[2] - most_similar_index[1] - bev_feat_base.shape[2]))
        # 将 pad_width_sequence 转换为元组
        pad_width = tuple(pad_width)
        # 对张量进行填充
        base_view_list = []
        for i in range(N):
            padded_tensor = np.pad(bev_feat_base_list[i].cpu().detach().numpy(), pad_width, mode='constant', constant_values=0)
            base_view_list.append(padded_tensor)
        base_view = np.stack(base_view_list, axis=0)
        bev_feat_chuli = []
        bev_feat_chuli.append(torch.tensor(base_view))
        for j in range(1, len(bev_feat)):
            for i in range(N):
                zoom_factor = (1, num, num)
                bev_feat_swap = bev_feat[j]
                bev_feat_swap_list = [bev_feat_swap[k, :, :, :] for k in range(N)]
                interpolated_tensor1 = zoom(bev_feat_swap_list[i].cpu().detach().numpy(), zoom_factor, order=1)
                bev_uav_list.append(interpolated_tensor1)
                bea_uav = np.stack(bev_uav_list, axis=0)
            bev_feat_chuli.append(torch.tensor(bea_uav))
            bev_uav_list = []
            bea_uav = []
        return bev_feat_chuli
    ...
    def extract_img_feat(self,
        ...
        for img, sensor2keyego, ego2global, intrin, post_rot, post_tran in zip(
            ...
            if key_frame or self.with_prev:
                ...
                mlp_input = self.img_view_transformer_veh.get_mlp_input(
